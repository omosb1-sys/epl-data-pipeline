...
# 일부 환경(샌드박스/제한된 런타임)에서 BLAS/OpenMP 다중 스레드가
# 드물게 비정상 종료를 유발할 수 있어 기본 스레드를 1로 제한합니다.
...
# matplotlib 캐시/설정 경로 및 백엔드 고정(권한 제한/헤드리스 환경 안정화)
...
from sklearn.metrics import (classification_report, accuracy_score, 
                            # ROC/AUC로 확률 예측 품질을 평가
                            roc_auc_score, roc_curve, confusion_matrix)
# 필요한 기능만 선택해서 불러오기(from import)
from sklearn.ensemble import RandomForestClassifier
# 라이브러리 불러오기(import)
import warnings
# 분석 파이프라인을 구성하는 실행 코드(흐름을 따라 읽기)
warnings.filterwarnings("ignore")

# ============================================================
# 🔧 기본 설정
# ============================================================
# 시각화(그래프) 관련 설정/그리기/저장
...
# 에러가 날 수 있는 구간을 try로 감싸 안전하게 처리
try:
    # CSV 파일을 읽어서 DataFrame으로 로드
    raw_data = pd.read_csv(PROJECT_DIR / "data/raw/raw_data.csv", encoding="utf-8")
    # CSV 파일을 읽어서 DataFrame으로 로드
    match_info = pd.read_csv(PROJECT_DIR / "data/raw/match_info.csv", encoding="utf-8")
    # 진행 상황/결과를 출력해서 현재 상태를 확인
...
# 그룹화(groupby)로 묶어서 요약 통계를 만들 준비
game_team_stats = df.groupby(['game_id', 'team_name_ko']).apply(
    ...
    lambda x: pd.Series({
        ...
        # 슈팅 지표
        # 분석 파이프라인을 구성하는 실행 코드(흐름을 따라 읽기)
        # 데이터에 따라 페널티 라벨이 'Penalty' 또는 'Penalty Kick'으로 다를 수 있어 둘 다 허용
        'total_shots': len(x[x['type_name'].isin(['Shot', 'Shot_Freekick', 'Penalty', 'Penalty Kick'])]),
        # 분석 파이프라인을 구성하는 실행 코드(흐름을 따라 읽기)
...
game_team_stats = game_team_stats.merge(
    ...
    # 변수 `on`에 계산 결과를 저장(다음 단계에서 재사용)
    on='game_id',
    # 변수 `how`에 계산 결과를 저장(다음 단계에서 재사용)
    how='left'
# 분석 파이프라인을 구성하는 실행 코드(흐름을 따라 읽기)
)
...
# 변수 `home_goals`에 계산 결과를 저장(다음 단계에서 재사용)
home_goals = game_team_stats[game_team_stats['is_home'] == 1]['goals']
# 변수 `away_goals`에 계산 결과를 저장(다음 단계에서 재사용)
...
# 조건에 따라 다른 로직을 실행하는 분기(조건문)
if p_ttest < 0.05:
    # 변수 `home_advantage`에 계산 결과를 저장(다음 단계에서 재사용)
...
# 변수 `team_goal_ranking`에 계산 결과를 저장(다음 단계에서 재사용)
...
# 조건에 따라 다른 로직을 실행하는 분기(조건문)
if p_corr < 0.05:
    # 변수 `direction`에 계산 결과를 저장(다음 단계에서 재사용)
...
# 변수 `corr_features`에 계산 결과를 저장(다음 단계에서 재사용)
...
# 상관관계 행렬 계산
# 변수 `correlation_matrix`에 계산 결과를 저장(다음 단계에서 재사용)
...
# 변수 `goal_correlations`에 계산 결과를 저장(다음 단계에서 재사용)
...
for var, corr in goal_correlations.items():
    # 조건에 따라 다른 로직을 실행하는 분기(조건문)
    if var != 'goals':
        # 변수 `strength`에 계산 결과를 저장(다음 단계에서 재사용)
...
sns.heatmap(correlation_matrix, annot=True, fmt='.2f', cmap='coolwarm', 
            # 변수 `center`에 계산 결과를 저장(다음 단계에서 재사용)
            center=0, square=True, linewidths=1, 
            # 변수 `cbar_kws`에 계산 결과를 저장(다음 단계에서 재사용)
...
# 변수 `feature_cols`에 계산 결과를 저장(다음 단계에서 재사용)
feature_cols = [
    # 분석 파이프라인을 구성하는 실행 코드(흐름을 따라 읽기)
...
# 로지스틱 회귀 모델 생성/학습(이진 분류 기본 모델)
lr_model = LogisticRegression(max_iter=1000, random_state=42)
# 분석 파이프라인을 구성하는 실행 코드(흐름을 따라 읽기)
lr_model.fit(X_train_scaled, y_train)

# 변수 `y_pred_lr`에 계산 결과를 저장(다음 단계에서 재사용)
y_pred_lr = lr_model.predict(X_test_scaled)
# 변수 `y_pred_proba_lr`에 계산 결과를 저장(다음 단계에서 재사용)
y_pred_proba_lr = lr_model.predict_proba(X_test_scaled)[:, 1]

# 변수 `accuracy_lr`에 계산 결과를 저장(다음 단계에서 재사용)
...
print(classification_report(y_test, y_pred_lr, 
                          # 변수 `target_names`에 계산 결과를 저장(다음 단계에서 재사용)
                          target_names=['패배', '승리/무승부'],
                          # 변수 `zero_division`에 계산 결과를 저장(다음 단계에서 재사용)
                          zero_division=0))

# 피처 중요도 (계수)
# 변수 `feature_importance_lr`에 계산 결과를 저장(다음 단계에서 재사용)
...
# 랜덤포레스트 모델 생성/학습(비선형 패턴 학습)
rf_model = RandomForestClassifier(n_estimators=100, max_depth=10, 
                                  # 변수 `random_state`에 계산 결과를 저장(다음 단계에서 재사용)
                                  random_state=42, n_jobs=-1)
# 분석 파이프라인을 구성하는 실행 코드(흐름을 따라 읽기)
rf_model.fit(X_train, y_train)

# 변수 `y_pred_rf`에 계산 결과를 저장(다음 단계에서 재사용)
y_pred_rf = rf_model.predict(X_test)
# 변수 `y_pred_proba_rf`에 계산 결과를 저장(다음 단계에서 재사용)
y_pred_proba_rf = rf_model.predict_proba(X_test)[:, 1]

# 변수 `accuracy_rf`에 계산 결과를 저장(다음 단계에서 재사용)
...
# 피처 중요도
# 변수 `feature_importance_rf`에 계산 결과를 저장(다음 단계에서 재사용)
...
for idx, row in feature_importance_rf.iterrows():
    # 변수 `bar`에 계산 결과를 저장(다음 단계에서 재사용)
...
# 변수 `model_comparison`에 계산 결과를 저장(다음 단계에서 재사용)
...
# 변수 `best_model_name`에 계산 결과를 저장(다음 단계에서 재사용)
best_model_name = model_comparison.loc[model_comparison['AUC'].idxmax(), '모델']
# 변수 `best_auc`에 계산 결과를 저장(다음 단계에서 재사용)
...
# 변수 `cv_scores_lr`에 계산 결과를 저장(다음 단계에서 재사용)
...
# 변수 `cv_scores_rf`에 계산 결과를 저장(다음 단계에서 재사용)
...
# 변수 `team_stats_full`에 계산 결과를 저장(다음 단계에서 재사용)
